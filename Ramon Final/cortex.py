...
class Cortex():
    ...
    def sub_request_mod(self, child_conn, stream):
        print('subscribe request --------------------------------')
        sub_request_json = {
            "jsonrpc": "2.0", 
            "method": "subscribe", 
        ...
        if 'sys' in stream:
            ...
            while True:
                ...
                if "eeg" in data:
                    agg_eeg.append(np.array(data["eeg"][2:7]))
                else:
                    agg_pow.append(np.array(data["pow"]))
                if len(agg_eeg) == 2 * 128:
                    child_conn.send((agg_eeg,agg_pow))
                    agg_eeg = agg_eeg[128:]
                    agg_pow = agg_pow[8:]
